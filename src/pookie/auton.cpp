...
void win_point_auton() {
    ...
    pros::Task auton_task {[=] {
        ...
        turnToAngle(tank_drive_15, 124, 2);
        ...
        vector<vector<double>> toGoal = {toOtherDepot.back(), 
                                            {112, 12}, {126,16}, {127, 20}}; //pickup 2nd WP triball along path
        ...
        movePID(tank_drive_15, -6.5, 220, 1000);
        turnToAngle(tank_drive_15, 185, 2);
        
        for(int i=0; i<1; i++){
            delay(50);
            tank_drive_15.move_with_power(-100);
            waitUntil(tank_drive_15.getOdom().getY() >= convert::inToM(25));
        ...
        movePID(tank_drive_15, 4, 180, 1000);
        ...
        vector<vector<double>> sweep = {{convert::mToIn(tank_drive_15.getOdom().getX()), convert::mToIn(tank_drive_15.getOdom().getY())},
                                            {96,35}, {82.5, 40}, {83, 61}, {91, 63}, {96, 65}, {115, 65}}; //pickup 2nd WP triball along path
        ...
        //                                     {100, 34}, {107.5,29}, {87+6, 23.5}}; //pickup 2nd WP triball along path
        // move(toClimb, 270, false, false, 2, .35);
        turnToAngle(tank_drive_15, 235, 2);
