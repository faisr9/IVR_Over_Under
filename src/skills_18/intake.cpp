...
void Intake::set_rpm(int rpm){
    //not sure on how to convert rpm to power
}
...
void Intake::toggle_on(int power = default_power_){
    intake_motor_->move_voltage(power);
}

void Intake::toggle_off(){ //this is a duplicate of subsystem parent stop ------------------------
    intake_motor_->brake();
}