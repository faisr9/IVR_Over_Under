...
void initialize() {
	...
	if(!usd::is_installed()) // No SD card installed 
		rtype_warn = 3;
	else if(fileExists(SKILLS_18_CHECKFILE)) // If file exists, check value
	{
		FILE *skills_18_check_r = fopen(SKILLS_18_CHECKFILE, "r");
		char checkValue[18];
		fread(checkValue, 1, 18, skills_18_check_r);
		fclose(skills_18_check_r);

		std::string checkValueStr(checkValue);
		if(strcmp(checkValueStr.c_str(), SKILLS_18_CHECKVALUE) != 0)
			rtype_warn = 2;	
	}
	...
	if(rtype_warn==1)
	{
		...
		lcd::print(6, "Robot Type: Skills 18 Bot");
		...
		lcd::print(1, "Skills 18 Bot");
		FILE *skills_18_check_w = fopen(SKILLS_18_CHECKFILE, "w");
		fputs(SKILLS_18_CHECKVALUE, skills_18_check_w);
		fclose(skills_18_check_w);
		delay(2500);
		// lcd::shutdown();
	}
