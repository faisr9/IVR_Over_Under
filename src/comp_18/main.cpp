...
void initialize() {
	...
	if(!usd::is_installed()) // No SD card installed 
		rtype_warn = 3;
	else if(fileExists(COMP_18_CHECKFILE)) // If file exists, check value
	{
		FILE *comp_18_check_r = fopen(COMP_18_CHECKFILE, "r");
		char checkValue[15];
		fread(checkValue, 1, 15, comp_18_check_r);
		fclose(comp_18_check_r);

		std::string checkValueStr(checkValue);
		if(strcmp(checkValueStr.c_str(), COMP_18_CHECKVALUE) != 0)
			rtype_warn = 2;	
	}
	...
	if(rtype_warn==1)
	{
		...
		lcd::print(6, "Robot Type: Comp 18 Bot");
		...
		lcd::print(1, "Comp 18 Bot");
		FILE *comp_18_check_w = fopen(COMP_18_CHECKFILE, "w");
		fputs(COMP_18_CHECKVALUE, comp_18_check_w);
		fclose(comp_18_check_w);
		delay(2500);
		// lcd::shutdown();
	}
