...
void auton18() {
	...
	std::vector<std::vector<double>> move1 = {start, vect(3.5, 0.5)}; //Movement 1 (Move to intake depot)
	std::vector<std::vector<double>> move2 = {move1.back(), vect(4.8, .8)}; // Movement 2 (Turn to flick triball)
    std::vector<std::vector<double>> move3 = {move2.back(), vect(5.5, 1.3)}; // Movement 3 (Drive forward to score)
    ...
    move(move3, 30, false, false);
    turnToAngle(tank_drive_18, 5, 3.0,false, 1.12);
...
void move(std::vector<std::vector<double>> moveVec, int angle, bool isReversed, bool isSpinAtEnd){
    followPath(moveVec, tank_drive_18, angle, isReversed, false,  false, 0.5, 3.0, 200.0/2, 450.0/2, 40.0/2, false);
}