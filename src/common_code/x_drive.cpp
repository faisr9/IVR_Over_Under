...
void x_drive::robot_centric_move(std::pair<double, double> movement_vector, double right_x){
    ...
    if (angle == 0){
        ...
        move_x = magnitude * cos(angle) / sqrt(2.0);
        move_y = magnitude * sin(angle) / sqrt(2.0);
    }

    double kmultiplier = .9;
    double left_move = kmultiplier*(move_y - move_x);
    double right_move = kmultiplier*(move_y + move_x);
    
    front_left_.move(right_move + right_x);
    front_right_.move(left_move - right_x);
    back_left_.move(left_move + right_x);
    back_right_.move(right_move - right_x);

}

void x_drive::field_centric_move(std::pair<double, double> movement_vector, double right_x){
    ...
    this->robot_centric_move(new_movement_vector,right_x);
}
