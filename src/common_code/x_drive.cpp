...
x_drive::x_drive(Controller master, Motor front_left, Motor front_right, Motor back_left, Motor back_right, Imu imu) : 
    master_(master), front_left_(front_left), front_right_(front_right), back_left_(back_left), back_right_(back_right) {}
...
void x_drive::robot_centric_move(std::pair<double, double> movement_vector){
    ...
    double move_y = magnitude * cos(angle) / sqrt(2.0);
    ...
    //b(toward left) = (y-x)/root2 while a(toward right) = (y+x)/root2
...
void x_drive::field_centric_move(std::pair<double, double> movement_vector){
    ...
    IMU imu_ = IMU(1); //temp imu thing so I can write code
