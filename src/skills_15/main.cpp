...
void initialize() {
	...
	if(!usd::is_installed()) // No SD card installed 
		rtype_warn = 3;
	else if(fileExists(SKILLS_15_CHECKFILE)) // If file exists, check value
	{
		FILE *skills_15_check_r = fopen(SKILLS_15_CHECKFILE, "r");
		char checkValue[19];
		fread(checkValue, 1, 19, skills_15_check_r);
		fclose(skills_15_check_r);

		std::string checkValueStr(checkValue);
		if(strcmp(checkValueStr.c_str(), SKILLS_15_CHECKVALUE) != 0)
			rtype_warn = 2;	
	}
	...
	if(rtype_warn==1)
	{
		...
		lcd::print(6, "Robot Type: Skills 15 Bot");
		...
		FILE *skills_15_check_w = fopen(SKILLS_15_CHECKFILE, "w");
		fputs(SKILLS_15_CHECKVALUE, skills_15_check_w);
		fclose(skills_15_check_w);
		delay(2500);
		// lcd::shutdown();
	}
